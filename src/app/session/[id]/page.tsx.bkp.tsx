...
export default function SessionPage({ params }: { params: { id: string } }) {
  ...
  async function loadStep() {
    ...
    const r = await fetch(
      `/api/step?sessionId=${encodeURIComponent(sessionId)}`
    );
  ...
  async function send() {
    ...
    // Make sure we SEE the user's message immediately.
    ...
    try {
      ...
      if (!r.ok) {
        setMsgs((m) => [
          ...m,
          { role: "agent", text: d.error || "Server error" },
        ]);
        return;
      }
      ...
      if (d.type === "answer" && d.text) {
        ...
        setMsgs((m) => [
          ...m,
          { role: "agent", text: JSON.stringify(d) || "(empty reply)" },
        ]);
      }
    } catch (e: any) {
  ...
  return (
    ...
    <main style={{ maxWidth: 900, margin: "0 auto", padding: 12 }}>
      <div style={{ color: "#555", marginBottom: 6 }}>{step?.title}</div>
